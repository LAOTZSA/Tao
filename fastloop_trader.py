...
def run_fast_market_strategy(dry_run=True, positions_only=False, show_config=False,
    ...
    log("â¡ Simmer FastLoop Trading Skill")
    log("=" * 50)
    ...
    log(f"  Volume weighting: {'â' if VOLUME_CONFIDENCE else 'â'}")
    ...
    log(f"\nð Discovering {ASSET} fast markets...")
    ...
    if not markets:
        ...
        if not quiet:
            print("ð Summary: No markets available")
        return
    ...
    if not best:
        ...
        if not quiet:
            print("ð Summary: No tradeable fast_markets (too close to expiry)")
        return

    end_time = best.get("end_time")
    remaining = (end_time - datetime.now(timezone.utc)).total_seconds() if end_time else 0
    log(f"\nð¯ Selected: {best['question']}")
    ...
    log(f"\nð Fetching {ASSET} price signal ({SIGNAL_SOURCE})...")
    momentum = get_momentum(ASSET, SIGNAL_SOURCE, LOOKBACK_MINUTES)

    if not momentum:
        log("  â Failed to fetch price data", force=True)
        return
    ...
    if momentum_pct < MIN_MOMENTUM_PCT:
        log(f"  â¸ï¸  Momentum {momentum_pct:.3f}% < minimum {MIN_MOMENTUM_PCT}% â skip")
        if not quiet:
            print(f"ð Summary: No trade (momentum too weak: {momentum_pct:.3f}%)")
        return

    # Calculate expected fair price based on momentum direction
    # Simple model: strong momentum â higher probability of continuation
    if direction == "up":
        side = "yes"
    ...
    if VOLUME_CONFIDENCE and momentum["volume_ratio"] < 0.5:
        log(f"  â¸ï¸  Low volume ({momentum['volume_ratio']:.2f}x avg) â weak signal, skip")
        if not quiet:
            print(f"ð Summary: No trade (low volume)")
        return
    elif VOLUME_CONFIDENCE and momentum["volume_ratio"] > 2.0:
        vol_note = f" ð (high volume: {momentum['volume_ratio']:.1f}x avg)"

    # Check divergence threshold
    if divergence <= 0:
        log(f"  â¸ï¸  Market already priced in: divergence {divergence:.3f} â¤ 0 â skip")
        if not quiet:
            print(f"ð Summary: No trade (market already priced in)")
        return
    ...
    if fee_rate > 0:
        ...
        if divergence < min_divergence:
            log(f"  â¸ï¸  Divergence {divergence:.3f} < fee-adjusted minimum {min_divergence:.3f} â skip")
            if not quiet:
                print(f"ð Summary: No trade (fees eat the edge)")
            return
    ...
    if remaining_budget <= 0:
        ...
        if not quiet:
            print(f"ð Summary: No trade (daily budget exhausted)")
        return
    ...
    if position_size < 0.50:
        ...
        if not quiet:
            print(f"ð Summary: No trade (remaining budget too small)")
        return
    ...
    if price > 0:
        ...
        if min_cost > position_size:
            log(f"  â ï¸  Position ${position_size:.2f} too small for {MIN_SHARES_PER_ORDER} shares at ${price:.2f}")
            return

    log(f"  â Signal: {side.upper()} â {trade_rationale}{vol_note}", force=True)
    log(f"  Divergence: {divergence:.3f}", force=True)

    # Step 5: Import & Trade
    log(f"\nð Importing to Simmer...", force=True)
    market_id, import_error = import_fast_market_market(best["slug"])

    if not market_id:
        log(f"  â Import failed: {import_error}", force=True)
        return

    log(f"  â Market ID: {market_id[:16]}...", force=True)
    ...
    if dry_run:
        ...
        if result and result.get("success"):
            ...
            log(f"  â Bought {shares:.1f} {side.upper()} shares @ ${price:.3f}", force=True)
            ...
            log(f"  â Trade failed: {error}", force=True)
...
if __name__ == "__main__":
    ...
    if args.set:
        ...
        print(f"â Config updated: {json.dumps(updates)}")
        sys.exit(0)
